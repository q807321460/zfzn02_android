...
import com.jia.camera.common.RecoderSeekBar;
	...
	class MyBasePlayerEventListener extends LCOpenSDK_EventListener {
		...
		public void onPlayerResult(int index, String code, int type) {
			...
			if(type == Business.RESULT_SOURCE_OPENAPI){
				...
				return;
			}else if(code.equals(Business.PlayerResultCode.STATE_RTSP_TEARDOWN_ERROR)) {
				if (mHander != null) {
					mHander.post(new Runnable() {
						@Override
						public void run() {
			...
			}else if(code.equals(Business.PlayerResultCode.STATE_PACKET_FRAME_ERROR) ||
...
//		}		
	...
	private void playSeek(long startTime, long endTime) {
		...
		mPlayWin.playRtspPlaybackByUtcTime(Business.getInstance().getToken(), 
				channelInfo.getDeviceCode(), channelInfo.getIndex(), startTime, endTime);
	}
	...
	public void play(int strRes) {
		...
		mRecordPlayPause.setImageResource(R.drawable.record_btn_pause);
		if (recordInfo.getType() == RecordInfo.RecordType.PublicCloud){
			mPlayWin.playCloud(Business.getInstance().getToken(), channelInfo.getDeviceCode(), recordInfo.getRecordID());
		}else{
			...
			mPlayWin.playRtspPlaybackByFileName(Business.getInstance().getToken(), channelInfo.getDeviceCode(), recordInfo.getRecordPath());
		}
	}
