package com.jia.camera.business.util;
...
public class ImageHelper {
	 ...
	 * @param handler
	 */
	public static void loadRealImage(final String url, String key, final Handler handler) {
		downloadImage(url, "real", key, handler);
	}
	 ...
	 * @param handler
	 */
	public static void loadCacheImage(final String url, String key, final Handler handler) {
		String[] imageIDBuffer = url.split("[/?]");
		final String imageID = imageIDBuffer[imageIDBuffer.length - 2];
		Drawable drawable = mImageCache.get(imageID);
		if(drawable != null){
			...
			downloadImage(url, imageID, key, handler);
		}
	}
	 ...
	 * @param handler
	 */
	private static void downloadImage(final String url, final String imageID, final String key, final Handler handler) {
		TaskPoolHelper.addTask(new TaskPoolHelper.RunnableTask(imageID) {
			@Override
			public void run() {
				// TODO Auto-generated method stub
				...
				try {  
                    ...
                    int dstLen = 500000;
                    Bitmap bitmap; //=null
                    switch(LCOpenSDK_Utils.decryptPic(srcBuf,length,key,dstBuf,dstLen)){
                    case 0:  //解密成功
                    	bitmap = BitmapFactory.decodeByteArray(dstBuf, 0, dstLen, mDefaultOption);
                    	if (bitmap != null) {
                    		drawable = new BitmapDrawable(bitmap);
                    	}
                    	break;
