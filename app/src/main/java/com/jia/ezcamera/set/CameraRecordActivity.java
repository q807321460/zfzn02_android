...
import vv.ppview.PpviewClientInterface.OnC2dSDCardCallback;
...
public class CameraRecordActivity extends Activity {
    ...
    OnClickListener otherOnClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.btn_return:
				finish();
				break;
			case R.id.record_sdcard_format:
            	formatSDcard();
                break;
    ...
    OnClickListener setOnClickListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            switch (v.getId()) {
                case R.id.record_timing:
                	intent.setClass(mContext, CameraRecordTimeActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        init();
        onvif_c2s.setOnC2dSDCardCallback(onC2dSDCardCallback);
        onvif_c2s.c2d_getSDInfo_fun(app.SetCamConnector);
    ...
    private void init() {
        ...
        sdcard_info = (TextView)findViewById(R.id.record_sdcard_info);
        
        findViewById(R.id.record_sdcard_format).setOnClickListener(otherOnClickListener);
        findViewById(R.id.record_timing).setOnClickListener(setOnClickListener);
        findViewById(R.id.record_alarm).setOnClickListener(setOnClickListener);
        ...
        swichButton.setOnClickListener(setOnClickListener);
        findViewById(R.id.record_set_time_startlayout).setOnClickListener(setOnClickListener);
        findViewById(R.id.record_set_time_endlayout).setOnClickListener(setOnClickListener);
    ...
    private void formatSDcard(){
    	...
    	dialog = new BottomDialog(mContext, getString(R.string.record_sdcard_format_hint), 
    			getString(R.string.record_sdcard_format), 
					...
					public void onClick(View v) {
						...
						ProgressDialogUtil.getInstance().showDialog(
								...
								mContext.getResources().getString(
										R.string.record_sdcard_formating),
								true);
						onvif_c2s.c2d_formatSdcard_fun(app.SetCamConnector);
						dialog.dismiss();
    ...
    OnC2dSDCardCallback onC2dSDCardCallback = new OnC2dSDCardCallback() {
		
		@Override
		public void on_c2d_getSdcardInfoCallBack(final int res, final item_sdcard_info info) {
			// TODO Auto-generated method stub
			runOnUiThread(new Runnable() {
				
				@Override
				public void run() {
					if(res == 200&&info!=null){
						if(info.status==1){
							String text = getString(R.string.total_space)+info.total_space+"MB"+getString(R.string.free_space)+info.free_space+"MB";
							sdcard_info.setText(text);
						}
						else if(info.status == 0 ){
							sdcard_info.setText(R.string.record_sdcard_noformat);
						}else{
							sdcard_info.setText(R.string.record_sdcard_err);
						}
					}else{
						sdcard_info.setText(R.string.record_sdcard_err);
					}
		...
		@Override
		public void on_c2d_formatSdcardCallBack(final int res) {
			// TODO Auto-generated method stub
			runOnUiThread(new Runnable() {
				public void run() {
					ProgressDialogUtil.getInstance().cancleDialog();
					if(res==200){
						ToastUtils.show(mContext, R.string.record_sdcard_format_succ);
						onvif_c2s.c2d_getSDInfo_fun(app.SetCamConnector);
					}else{
						ToastUtils.show(mContext, R.string.record_sdcard_format_fail);
					}
				}
			});
		}
		...
		@Override
		public void on_c2d_formatSdcardProCallBack(final int count) {
			// TODO Auto-generated method stub
			runOnUiThread(new Runnable() {
					public void run() {
						ProgressDialogUtil.getInstance().setDialogText(
							getString(R.string.record_sdcard_formating)+"("+count+"%"+")");
				}
			});
